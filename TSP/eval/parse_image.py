...
def parse_tsp_image(image_path, grid_size, tolerance=0.70):
    ...
    def draw_middle_segment(mask, p1, p2, thickness, keep_ratio=0.7):
        x1, y1 = p1
        x2, y2 = p2
        
        mx1 = int(x1 + (x2 - x1) * (1 - keep_ratio) / 2)
        my1 = int(y1 + (y2 - y1) * (1 - keep_ratio) / 2)
        mx2 = int(x1 + (x2 - x1) * (1 + keep_ratio) / 2)
        my2 = int(y1 + (y2 - y1) * (1 + keep_ratio) / 2)
        
        cv2.line(mask, (mx1, my1), (mx2, my2), 255, thickness)
    ...
    for i in range(num_nodes):
        for j in range(i + 1, num_nodes):
            ...
            draw_middle_segment(test_mask, sp, ep, probe_thickness, keep_ratio=0.7)
