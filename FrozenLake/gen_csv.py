...
def process_single_directory(root_dir, is_final):
    ...
    prefix_prompt = "Draw a continuous red line connecting the Start point to the Goal point step-by-step, avoiding all holes. Mark the current end of the path with a green dot. The line must be drawn through the center of the grid cells rather than along the grid lines. Direction keys: D=Down, U=Up, R=Right, L=Left."
    ...
    for folder_name in sorted(subdirs):
        ...
        current_image_ref = os.path.join(root_dir, folder_name, f"{folder_name}.png")
