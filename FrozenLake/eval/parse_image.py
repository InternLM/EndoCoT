...
def process_single_level(level):
    if level >= 16:
        BASE_DIR = f"/path/to/the/FrozenLake/VSP/{level}_test"
        RESULT_DIR = f"{BASE_DIR}/result"
        TABLE_DIR = f"{BASE_DIR}/table"
    else:
        RESULT_DIR = f"/path/to/the/FrozenLake/VSP/maps/level{level}/img/result"
        TABLE_DIR = f"/path/to/the/FrozenLake/VSP/maps/level{level}/table"
